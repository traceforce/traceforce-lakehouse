...
- The findings tables say what was found, where, when, and the triage state. They never contain the value.
- The matched sensitive *value* is in an evidence object outside the lake, referenced by the
  ...
  `GET /api/v1/sensitive-data-findings/{id}/content`, `/api/v1/connector-containment-findings/{id}/content` or `/api/v1/prompt-injection-findings/{id}/content`, which are authorized and audited.

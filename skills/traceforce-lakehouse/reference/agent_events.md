...
| `source_object` | string | Storage URI of the raw object this row came from — `s3://bucket/key` on AWS, `gs://bucket/key` on GCP. Equals concat(scheme, '://', bucket, '/', key_prefix, source_file) of a finding's conversation_storage. |
...
| `resource_session_id` | string | OTLP resource attribute `session.id`, a fallback source for `session_id`. Reserved: always NULL on events today (no agent sets it). |

...
| `agent_account_id` | string | → agent_accounts.id: the signed-in account this MCP configuration is scoped to. NULL when the config is scoped to the device/install rather than to an account — resolve those through device_id, or via mcp_server_agent_instances → agent_instances_accounts → agent_accounts (take the greatest agent_accounts.last_seen_at when several match). |

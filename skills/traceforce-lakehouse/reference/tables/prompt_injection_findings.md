...
One row per assistant message the agent flagged as a prompt-injection attempt. A detection record only: nothing was denied or blocked.
...
| `description` | string | Short, sanitized summary of the attack pattern the agent flagged: no user data, secrets or file contents (length-capped and scrubbed on the device). NULL when the flag carried none. The verbatim injection text is never in the lake; customer_storage points at it. |
| `detected_at` | timestamp | When the flag was read (UTC): the column to bound a period on; created_at is when TraceForce recorded it. Can be NULL. |

...
| `description` | string | Sanitized summary of the attack pattern; NULL when the flag carried none. The origin and quoted instruction are in the flagged reply's leading [PI_DETECTED source=… instruction=… description=…] line(s) in agent_events.content_output; customer_storage holds the same text as an evidence object. |

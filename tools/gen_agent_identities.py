...
newer than the pinned proto tag lands with agent_type NULL until this is regenerated:
    ...
    tools/gen_agent_identities.py     # bump PINNED below first, so the script and the JSON always agree
...
src = subprocess.check_output(["git", "-C", str(proto_dir), "show", f"{PINNED}:proto/scout.proto"], text=True)
body = re.search(r"enum AgentIdentity\s*\{(.*?)\n\}", src, re.S).group(1)
entries = re.findall(r"^\s*(AGENT_IDENTITY_[A-Z0-9_]+)\s*=\s*(\d+)", body, re.M)
out = {
    "_generated": f"tools/gen_agent_identities.py from traceforce-scout-proto {PINNED} proto/scout.proto (enum AgentIdentity). Do not edit by hand.",
...
print(f"{len(entries)} identities from {PINNED}")

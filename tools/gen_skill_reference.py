...
TABLES = {
    ...
    "connector_containment_findings": dict(
        ...
        notes={"metadata": "JSON text; reviewer close tracking {statusComment, statusUpdatedAt, statusUpdatedBy}; '{}' or NULL until a reviewer closes the finding.",
               "conversation_id": "→ agent_conversations.id", "op": "Kind of mutation: always 'write' or 'delete' (never NULL; denied/unknown are filtered out).", "tool_name": "The agent's shell tool (e.g. `Bash`, `Shell`) or `MCP:<server>`; never a built-in file tool (Write/Edit).",
               ...
               "hook_event_name": "The agent's own event name for the record that observed it (varies by agent, e.g. `tool_result`, `postToolUse`); never a pre-execution hook.", "detected_at": "When the op was observed (UTC); the reliable order-by time for containment findings; effectively always set.",

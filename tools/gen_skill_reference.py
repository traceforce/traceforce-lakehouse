...
TABLES = {
    ...
    "agent_instances": dict(
        ...
        notes={"metadata": "JSON text; how the install was identified: identifier, store_id, heuristic_match_method, heuristic_url, store_error.",
               ...
               "agent_catalog_id": "→ agent_catalog.id (FK); may be NULL — join agent_catalog on agent_type instead.", "configuration": "JSON text; detected agent configuration. Reserved: always '{}' today.",
    ...
    "connector_containment_findings": dict(
        ...
        notes={"metadata": "JSON text; reviewer close tracking {statusComment, statusUpdatedAt, statusUpdatedBy}; '{}' or NULL until a reviewer closes the finding.",
               "conversation_id": "→ agent_conversations.id", "op": "Kind of mutation: always 'write' or 'delete' (never NULL; denied/unknown are filtered out).", "tool_name": "Tool that attempted it: 'Bash', 'Shell' (Cursor) or 'MCP:<server>'; never a built-in file tool (Write/Edit).",
               "operation": "Display-safe subject of the op: an MCP tool's bare name, or a file path, redacted and truncated to 512 bytes; '' (empty string) for shell/command ops (Bash/Shell); the verbatim command never lands here, only in customer_storage — read the actual tool input from the joined agent_events.tool_args.",
               ...
               "hook_event_name": "Event name of the record that observed it: 'tool_result' / 'tool_decision' (Claude-family) or 'postToolUse' / 'postToolUseFailure' (Cursor).", "detected_at": "When the op was observed (UTC); the reliable order-by time for containment findings; effectively always set.",
               ...
               "risky_action_category": "Grouping of the risky action, as text: 'company_data_or_infra_destroyed' / 'company_systems_changed' / 'device_defenses_disabled_or_wiped' / 'device_or_dev_env_changed'; 'unknown' on rows from older scouts (no taxonomy). Derived from risky_action_subcategory.",
    ...
    "prompt_injection_findings": dict(
        ...
        notes={"metadata": "JSON text; reviewer close tracking {statusComment, statusUpdatedAt, statusUpdatedBy}; '{}' or NULL until a reviewer closes the finding.",
               ...
               "description": "Short, sanitized summary of the attack pattern the agent flagged: no user data, secrets or file contents (length-capped on the device). NULL when the flag carried none. The origin label and the quoted instruction are in the flagged reply itself: its leading [PI_DETECTED source=… instruction=… description=…] line(s) in agent_events.content_output, masked only where a sensitive value matched; customer_storage points at the same text as an evidence object.",
    ...
    "mcp_server_instances": dict(
        ...
        notes={"metadata": "JSON text; detection details for filesystem MCPs: matched_pattern, auth_matched_pattern, config_type, uid, version, agent_identity, agent_deployment. Browser connectors carry none of these.",
               "agent_account_id": "→ agent_accounts.id: the account a browser-connector MCP belongs to. NULL for filesystem-discovered MCPs — attribute those via mcp_server_agent_instances → agent_instances_accounts → agent_accounts (take the greatest agent_accounts.last_seen_at when several match).", "mcp_server_id": "→ mcp_servers.id (org rollup).",
               "mcp_server_type": "Integer product code; LEFT JOIN the catalogs for the name (0 = unknown, no catalog row). Not an enum.",
               "mcp_server_location": "Where it was found, never a URL or command: the config file path (e.g. mcp.json) for filesystem MCPs, the agent's web domain (e.g. .claude.ai) for browser connectors.",
    ...
    "mcp_servers": dict(
        purpose="Org-level rollup per MCP product: counts and scores. One row per product per org (the console's status column is not mirrored).",
        ...
        notes={"mcp_catalog_id": "→ mcp_catalog.id (FK); NULL for products without a global catalog row", "mcp_server_type": "Product code; same as the catalogs' mcp_server_type.",
               "active_users": "MISNOMER: despite the name, the count of DISTINCT active devices running this MCP type (devices, not users); 0 or NULL when the org has none.", "total_incidents": "Count of open issue rows for this type (not deduped by type); 0 or NULL when none are open.",
               ...
               "has_host_instances": "Any instance on a host (not only sandboxes).", "first_seen_at": "First observed (UTC).", "last_reviewed": "Last reviewed (UTC). Reserved: always NULL today."}),
    "mcp_catalog": dict(
...
EVENT_NOTES = {
    ...
    "source_object": "Storage URI of the raw object this row came from — `s3://bucket/key` on AWS, `gs://bucket/key` on GCP. Equals concat(scheme, '://', bucket, '/', key_prefix, source_file) of a finding's conversation_storage.",
    ...
    "resource_session_id": "OTLP resource attribute `session.id`, a fallback source for `session_id`. Reserved: always NULL on events today (no agent sets it).",
    "ts": "Event time, UTC.",
    ...
    "generation_id": "Per-turn identifier for agents that emit one instead of `prompt_id` (Cursor); NULL otherwise.",
    ...
    "tool_result": "Tool output; NULL when the agent doesn't emit it. Claude Code: on the span row only, never on its `tool_result` rows. Large.",
    ...
    "model": "The model requested for the turn; `default` (Cursor) and `auto` (Copilot) mean auto-select, not a model name.",

...
finding_status = 'awaiting_review'); there is no integer-decode ring here. The only code
left is the join key mcp_server_type, resolved via the catalogs.
"""
...
UNIQUE = {
    ...
    "prompt_injection_findings": ["(org_id, conversation_id, message_external_id, description), NULLs compared as equal: one row per flagged message per distinct description"],
...
TABLES = {
    "devices": dict(
        purpose="One row per device TraceForce has seen (laptops, workstations).",
        ...
        notes={"device_native_id": "OS-reported serial (e.g. C02XXXXXXXXX on macOS). Same value as agent_events.device_native_id. Windows serials can be placeholders shared by many machines.",
               ...
               "account_metadata": "JSON text keyed by OS uid/SID; each value is {username, home_directory}. Resolve agent_instances.tenant with json_extract_scalar(account_metadata, '$[\"<tenant>\"].username')."}),
    "sandboxes": dict(
        purpose="Devcontainers / cloud VMs an agent ran in.",
    ...
    "agent_accounts": dict(
        purpose="One row per signed-in account: (device, agent, email, vendor org).",
        ...
        notes={"metadata": "JSON text; how the account was discovered: source_identifier, source_deployment_type, user_id.",
               "agent_type": "Which agent product.", "plan": "Account plan (decoded AgentPlanType text, e.g. 'personal', 'enterprise', 'small_business'; 'unknown' for AgentPlanType 0). Counting agents: reference/identity.md.", "tier": "Vendor plan tier text when known.",
               "agent_email": "Email the user authenticated to the agent with. Stored byte-exact; lower() is a tolerance.",
               "agent_org_id": "The AI vendor's workspace/org id (e.g. the Anthropic org for Claude); NULL or '' when the agent has no workspace/org concept.",
    ...
    "agent_instances": dict(
        purpose="Installs: one row per (device, agent, deployment form factor, OS user). Includes agents that never produce an account (GitHub Copilot).",
        ...
        notes={"metadata": "JSON text; how the install was identified: identifier, store_id, heuristic_match_method, heuristic_url, store_error.",
               "agent_type": "Which agent product.", "agent_deployment": "Form factor of this install, as text ('desktop_app' / 'vscode_extension' / 'cli' / 'browser' / 'ai_browser' / 'service' / 'unknown').",
               "tenant": "OS-level user (uid on macOS/Linux, SID on Windows) the install belongs to; resolve to a username via devices.account_metadata. For agent_deployment = 'browser' it is the agent's web domain instead (does not resolve).",
               "agent_catalog_id": "→ agent_catalog.id (FK); may be NULL — join agent_catalog on agent_type instead.", "configuration": "JSON text; always '{}'.",
               "installations": "JSON text {installations: [{name, path, version}, …]}; '{}' when agent_deployment = 'browser'."}),
    ...
    "device_owner_mappings": dict(
        purpose="Device owner from your MDM.",
    ...
    "agent_catalog": dict(
        ...
        notes={"agent_name": "Display name (Claude Code, Cursor, GitHub Copilot, ChatGPT, ...).", "agent_type": "Join key.",
               "domain": "Vendor domain."}),
    "sensitive_data_findings": dict(
        purpose="One row per sensitive-data match (a credential, PII value, ...) found in a prompt/response or an attached file. A match is in an attachment when file_id is set, and in message text when file_id IS NULL.",
        joins=["person: conversation_id → agent_conversations.agent_account_id → agent_accounts.agent_email (NOT NULL; every finding has one; no deleted_at filter). Agent name: that agent_accounts row's agent_type → agent_catalog (findings carry no agent_type)",
        ...
        notes={"metadata": "JSON text; reviewer close tracking {statusComment, statusUpdatedAt, statusUpdatedBy}; '{}' or NULL until a reviewer closes the finding.",
               "conversation_id": "→ agent_conversations.id (TraceForce uuid, NOT the agent's session id).",
               ...
               "start_offset": "Character offset of the match start from the start of start_line.", "end_offset": "Character offset of the match end (exclusive) from the start of end_line; may be < start_offset when end_line > start_line.",
               "start_line": "0-indexed first line of the match within the scanned text (message or attachment).", "end_line": "0-indexed last line (inclusive).",
    ...
    "connector_containment_findings": dict(
        purpose="One row per risky write/delete a tool attempted through a connector (Bash, Shell, MCP). Denied attempts are NOT stored; only executed/failed ones.",
        ...
        notes={"metadata": "JSON text; reviewer close tracking {statusComment, statusUpdatedAt, statusUpdatedBy}; '{}' or NULL until a reviewer closes the finding.",
               "conversation_id": "→ agent_conversations.id", "op": "Kind of mutation: 'write' or 'delete' (never NULL).", "tool_name": "The agent's shell tool (e.g. `Bash`, `Shell`) or `MCP:<server>`.",
               "operation": "Display-safe subject of the op: an MCP tool's bare name, or a file path, redacted and truncated to 512 bytes; '' for shell ops (Bash/Shell); the command itself is in the joined agent_events.tool_args.",
               "tool_use_id": "Agent's tool-call id; equals agent_events.tool_call_id.", "prompt_id": "Always NULL here; take prompt_id from the joined agent_events row (tool_use_id = tool_call_id).",
               "hook_event_name": "The agent's own event name for the record that observed it (varies by agent, e.g. `tool_result`, `postToolUse`).", "detected_at": "When the op was observed (UTC); bound periods on this, not created_at.",
               "finding_status": "Reviewer triage state as text (same set as sensitive_data_findings.finding_status); never NULL, starts 'awaiting_review'.", "outcome": "Execution result as text ('executed' / 'failed' / 'unspecified'); never NULL.",
               "risky_action_category": "Grouping of the risky action, as text: 'company_data_or_infra_destroyed' / 'company_systems_changed' / 'device_defenses_disabled_or_wiped' / 'device_or_dev_env_changed'; 'unknown' on rows from older scouts.",
               "risky_action_subcategory": "The specific risky action, as text, grouped by category — destroyed: 'shared_db_destroyed' / 'cloud_infra_destroyed' / 'connector_records_deleted'; changed: 'cloud_resources_changed' / 'shared_db_written' / 'shared_repo_history' / 'connector_records_written' / 'connector_commands_run'; device defenses: 'core_protection_off' / 'device_wiped' / 'downloaded_code_run' / 'protection_loosened'; device or dev environment: 'processes_services_stopped' / 'device_settings_changed' / 'software_installed' / 'local_work_discarded' / 'local_db_changed' / 'local_connector_writes'; 'unknown' on rows from older scouts."}),
    "prompt_injection_findings": dict(
        purpose="One row per distinct prompt-injection attempt (assistant reply + sanitized description) the agent flagged. A detection record only: nothing was denied or blocked.",
        joins=["person: conversation_id → agent_conversations.agent_account_id → agent_accounts.agent_email (NOT NULL; every finding has one; no deleted_at filter). Agent name: that agent_accounts row's agent_type → agent_catalog (findings carry no agent_type)",
               "device owner (secondary, for display): device_id → devices.device_native_id → device_owner_mappings.owner_email",
        ...
        notes={"metadata": "JSON text; reviewer close tracking {statusComment, statusUpdatedAt, statusUpdatedBy}; '{}' or NULL until a reviewer closes the finding.",
               "conversation_id": "→ agent_conversations.id (TraceForce uuid, NOT the agent's session id).",
               "message_external_id": "The agent's own id of the assistant reply that carried the flag (a Claude response id; only Claude-family agents raise these findings).",
               "description": "Sanitized summary of the attack pattern; NULL when the flag carried none. The origin and quoted instruction are in the flagged reply's leading [PI_DETECTED source=… instruction=… description=…] line(s) in agent_events.content_output; customer_storage holds the same text as an evidence object.",
               "detected_at": "When the flagged reply was sent (UTC): the column to bound a period on; created_at is when TraceForce recorded it.",
               "finding_status": "Reviewer triage state (text: awaiting_review, under_review, false_positive, revoked, used_in_tests, wont_fix, acknowledged, unknown). Open, as the API counts it, is finding_status IN ('awaiting_review', 'under_review')."}),
    "agent_conversations": dict(
        purpose="One row per agent session/conversation TraceForce has scanned; created on scan whether or not anything was found. Count findings from the findings tables, not from here.",
        joins=["agent_conversations.conversation_external_id = agent_events.session_id", "agent_account_id → agent_accounts.id", "device_id → devices.id",
               "customer_storage.source_file is not a join key to agent_events; use conversation_external_id."],
    ...
    "agent_conversation_files": dict(
        purpose="Files attached to conversations (uploads).",
        ...
        notes={"conversation_id": "→ agent_conversations.id", "file_external_id": "Agent's own file id.", "file_name": "Original filename.",
               ...
               "message_timestamp": "When it was attached (UTC).", "archive_inner_path": "Entry path when the file is an archive member; '' (never NULL) for a plain file."}),
    "mcp_server_instances": dict(
        purpose="MCP servers configured on a device for an agent.",
        joins=["lower(mcp_server_instances.mcp_native_id) = lower(agent_events.mcp_server_name) AND mcp_server_instances.agent_type = agent_events.agent_type, on the same device via mcp_server_instances.device_id → devices.device_native_id = agent_events.device_native_id. Not a key: the same server name can have several live rows per device (one per project_path or location), so aggregate or SELECT DISTINCT",
        ...
        notes={"metadata": "JSON text; detection details for filesystem MCPs: matched_pattern, auth_matched_pattern, config_type, uid, version, agent_deployment.",
               "agent_account_id": "→ agent_accounts.id (browser connectors); NULL for filesystem MCPs — see Joins.", "mcp_server_id": "→ mcp_servers.id (org rollup).",
               "mcp_server_type": "Integer product code; LEFT JOIN the catalogs for the name (0 = unknown, no catalog row).",
               "mcp_server_location": "Where it was found: the config file path (e.g. mcp.json) for filesystem MCPs, the agent's web domain (e.g. .claude.ai) for browser connectors.",
               ...
               "tools": "JSON object: '{}' until tools are discovered; '{\"tools\": [...]}' once they are ('[]' when none). Elements (json_extract(tools, '$.tools')) carry name, description and integer codes tool_status (1 enabled / 2 disabled), hitl_setting (1 allow_unsupervised / 2 always_ask / 3 blocked), permission_type (1 read / 2 write / 3 delete); 0 = unknown."}),
    ...
    "mcp_servers": dict(
        purpose="Org-level rollup per MCP product: counts and scores. One row per product per org.",
        joins=["mcp_catalog_id → mcp_catalog.id; for org-private products resolve by mcp_server_type against both catalogs",
               "mcp_server_instances.mcp_server_id = mcp_servers.id", "The console's MCP list shows only live rollups with active_users > 0 and mcp_catalog_id NOT NULL"],
        notes={"mcp_catalog_id": "→ mcp_catalog.id (FK); NULL for products without a global catalog row.", "mcp_server_type": "Product code; same as the catalogs' mcp_server_type.",
               "active_users": "Distinct active devices (not users) running this type; 0 or NULL when none.", "total_incidents": "Count of open issue rows for this type (not deduped by type); 0 or NULL when none are open.",
               "active_issues": "Count of distinct open issue types; 0 or NULL when none are open.", "affected_devices": "Count of distinct devices with an open issue of this type; 0 or NULL when none.", "base_score": "Baseline risk score (0-100) for the product; NULL (not 0) when no baseline scores exist yet.",
               "actual_score": "Risk score (0-100) from your org's usage; NULL (not 0) when no scores for this org+type yet.", "agent_names": "JSON array of agent display names with an active instance of this type; '[]' or NULL when there are none.",
               "distribution_channels": "JSON array of the distinct distribution_channel values of active instances of this type, same vocabulary as mcp_server_instances.distribution_channel ('platform' / 'tenant' / 'user' / 'unknown'); '[]' when none.",
               "auth_types": "JSON array of the distinct auth_type values of active instances of this type, same vocabulary as mcp_server_instances.auth_type ('oauth' / 'token' / 'basic_auth' / 'no_auth' / 'unknown'); '[]' when none.",
               "sandbox_runtime_types": "JSON array of the distinct runtime types of the sandboxes active instances run in, same vocabulary as sandboxes.runtime_type ('devcontainer' / 'unspecified'); '[]' when none run in a sandbox.",
               "instance_count": "Count of distinct active instances of this type; 0 or NULL when none.",
               "has_host_instances": "Any instance on a host (not only sandboxes).", "first_seen_at": "First observed (UTC).", "last_reviewed": "Last reviewed (UTC). Always NULL."}),
    "mcp_catalog": dict(
        ...
        notes={"mcp_server_name": "Product display name (Slack, Notion, Supabase, ...).", "mcp_server_type": "Product code used by instances and rollups.",
               "category_id": "→ mcp_categories.id; NULL until a category is assigned.", "source_type": "Publisher/provenance as text ('official' / 'community' / 'reference' / 'archived' / 'openai' / 'anthropic' / 'unknown'); never NULL, 'unknown' = unset.",
               "execution_environment": "JSON array of where the product runs, same vocabulary as mcp_server_instances.deployment_model ('local_process' / 'local_container' / 'local_service' / 'remote' / 'unknown'); '[]' when unset.",
               "authentication_methods": "JSON array of the auth methods the product supports, same vocabulary as mcp_server_instances.auth_type ('oauth' / 'token' / 'basic_auth' / 'no_auth' / 'unknown'); '[]' when unset.", "detection_patterns": "JSON text; how TraceForce recognizes it in configs."}),
    "org_mcp_catalog": dict(
        ...
        notes={"mcp_server_name": "Product display name.", "mcp_server_type": "Product code.", "category_id": "→ mcp_categories.id; NULL until a category is assigned.", "source_type": "Publisher/provenance as text ('official' / 'community' / 'reference' / 'archived' / 'openai' / 'anthropic' / 'unknown'); never NULL, 'unknown' = unset.",
               "execution_environment": "JSON array of where the product runs, same vocabulary as mcp_server_instances.deployment_model ('local_process' / 'local_container' / 'local_service' / 'remote' / 'unknown'); '[]' when unset.",
               "authentication_methods": "JSON array of the auth methods the product supports, same vocabulary as mcp_server_instances.auth_type ('oauth' / 'token' / 'basic_auth' / 'no_auth' / 'unknown'); '[]' when unset.", "detection_patterns": "JSON text."}),
    "mcp_categories": dict(
...
EVENT_NOTES = {
    ...
    "source_object": "Storage URI of the raw object this row came from — `s3://bucket/key` on AWS, `gs://bucket/key` on GCP. Equals a finding's conversation_storage URI.",
    "signal": "Whether the row is an OTLP `log` record or a `span`.",
    "user_email": "End-user email as reported by the agent; NULL when the agent reports none (see identity.md).",
    ...
    "session_id": "The conversation/session identifier. Joins agent_conversations.conversation_external_id.",
    "resource_session_id": "OTLP resource attribute `session.id`, a fallback source for `session_id`. Always NULL today.",
    "ts": "Event time, UTC.",
    ...
    "prompt_id": "Per-turn (request/response) identifier; NULL when the agent emits none.",
    "generation_id": "Per-turn identifier for agents that emit one instead of `prompt_id` (e.g. Cursor); NULL otherwise.",
    "tool_name": "The tool invoked. Claude Code MCP calls: the decision/result rows say `mcp_tool`, the span row of the same `tool_call_id` carries the MCP tool's name; the server is in `mcp_server_name`.",
    "tool_type": "Tool type (function, ...).",
    "tool_call_id": "Per-call id; equals connector_containment_findings.tool_use_id (join key). NULL when the agent emits none.",
    "mcp_server_name": "MCP server as configured on the device; joins lower(mcp_server_instances.mcp_native_id). Also set on Claude Code's `mcp_server_connection` rows (`operation` NULL), which are connections, not calls: filter `operation = 'execute_tool'` for calls.",
    "tool_args": "Tool input, JSON text; occasionally plain text (e.g. a raw shell command). Large.",
    "tool_result": "Tool output; NULL when the agent doesn't emit it. Some agents (e.g. Claude Code) put it on the span row only, not the `tool_result` log row. Large.",
    ...
    "model": "The model requested for the turn; selector values such as `default` or `auto` mean auto-select, not a model name.",
...
def main():
    ...
    lines = ["# TraceForce metadata tables", "",
             ...
             "the only integer code left is the join key `mcp_server_type` (resolve via the catalogs).",

...
TABLES = {
    ...
    "connector_containment_findings": dict(
        ...
        notes={"metadata": "JSON text; reviewer close tracking {statusComment, statusUpdatedAt, statusUpdatedBy}; '{}' or NULL until a reviewer closes the finding.",
               "conversation_id": "→ agent_conversations.id", "op": "Kind of mutation: always 'write' or 'delete' (never NULL; denied/unknown are filtered out).", "tool_name": "Tool that attempted it (Write, Bash, MCP:<server>).",
               ...
               "risky_action_category": "Rule-19 grouping of the risky action, as text: 'company_data_or_infra_destroyed' / 'company_systems_changed' / 'device_defenses_disabled_or_wiped' / 'device_or_dev_env_changed'; 'unknown' on rows from older scouts (no taxonomy). Derived from risky_action_subcategory.",
    ...
    "prompt_injection_findings": dict(
        ...
        notes={"metadata": "JSON text; reviewer close tracking {statusComment, statusUpdatedAt, statusUpdatedBy}; '{}' or NULL until a reviewer closes the finding.",
               ...
               "description": "Short, sanitized summary of the attack pattern the agent flagged: no user data, secrets or file contents (length-capped and scrubbed on the device). NULL when the flag carried none. The origin label and the quoted instruction are in the flagged reply itself: its leading [PI_DETECTED source=… instruction=… description=…] line(s) in agent_events.content_output, masked only where a sensitive value matched; customer_storage points at the same text as an evidence object.",
               "detected_at": "When the flagged reply was sent (UTC): the column to bound a period on; created_at is when TraceForce recorded it. Can be NULL.",
    ...
    "mcp_server_instances": dict(
        ...
        notes={"metadata": "JSON text; detection details: matched patterns, config_type, uid, version, agent_identity, agent_deployment, is_custom.",

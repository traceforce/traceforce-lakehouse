...
TABLES = {
    ...
    "prompt_injection_findings": dict(
        purpose="One row per assistant message the agent flagged as a prompt-injection attempt. A detection record only: nothing was denied or blocked.",
        joins=["person: conversation_id → agent_conversations.agent_account_id → agent_accounts.agent_email (NOT NULL; every finding has one). Do NOT filter agent_accounts.deleted_at here: findings on since-removed accounts still belong to that person. Agent name: that agent_accounts row's agent_type → agent_catalog (findings carry no agent_type)",
               "device owner (secondary, for display): device_id → devices.device_native_id → device_owner_mappings.owner_email",
               "conversation_id → agent_conversations.id → conversation_external_id = agent_events.session_id",
        ...
        notes={"conversation_id": "→ agent_conversations.id (TraceForce uuid, NOT the agent's session id).",
               "message_external_id": "The agent's own id of the source message (Claude prompt/response id, Cursor generation id).",
               "description": "Short, sanitized summary of the attack pattern the agent flagged: no user data, secrets or file contents (length-capped and scrubbed on the device). NULL when the flag carried none. The verbatim injection text is never in the lake; customer_storage points at it.",
               "detected_at": "When the flag was read (UTC): the column to bound a period on; created_at is when TraceForce recorded it. Can be NULL.",

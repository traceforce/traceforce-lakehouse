...
# data lives in (residency, and every 15-minute scan would otherwise cross regions).
...
locals {
  ...
  # Where TraceForce's collector writes activity objects, per agent:
  #   s3://<bucket>/<prefix>/conversations/<AGENT_IDENTITY_*>/<serial>/<email>[_<org>]/<session>/<ts>_<uuid>_logs|traces.json.gz
